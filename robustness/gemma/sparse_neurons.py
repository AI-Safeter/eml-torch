"""Frozen sparse-control procedure with Gemma's native GELU activation."""
    ...
    with torch.inference_mode():
        for split in ["train", "validation"]:
            ...
            activities[split] = torch.cat(
                [mlp.act_fn(mlp.gate_proj(x)) * mlp.up_proj(x) for x in mixed.split(128)]
            )
        mean, std = (
        ...
        for step in range(64):
            ...
            exported = (
                torch.nn.functional.gelu(mixed[:32] @ state["gate"].cuda().T, approximate="tanh")
