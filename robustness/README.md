...
The final audit hashes retained evidence and source files before and after
validation. The report and bundle must match that identity; packaging checks
again after copying. Report provenance also binds the rendered files to the
audited inputs. Relative paths preserve this identity when the bundle moves,
while checkpoint and metric caches can be regenerated by the relocated GPU
audit. The [integrity validation](bundle-integrity-validation.json) exercises
changed inputs, a changed report, relocation, and an exact GPU metric replay
after a trace roundtrip. These packaging fixtures do not replace the full-study
audit. Reproduce them in the Qwen environment with:

```bash
CUDA_VISIBLE_DEVICES=0 python validate_bundle.py --output /tmp/eml-bundle-integrity.json
```
