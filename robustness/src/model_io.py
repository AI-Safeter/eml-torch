...
def load_mlp(layer):
    ...
    classes = {"qwen3": Qwen3MLP, "llama": LlamaMLP}
    if config.model_type not in classes:
        raise ValueError(f"Unsupported model type: {config.model_type}")
    model = classes[config.model_type](config).float().cuda().eval().requires_grad_(False)
    model.load_state_dict(state)
    return model


def corrupt_row(row):
    return {**row, "a": row["c"], "b": row.get("corrupt_b", row["b"])}


def text(tok, row, corrupt=False):
    a = row["c"] if corrupt else row["a"]
    b = row.get("corrupt_b", row["b"]) if corrupt else row["b"]
    ...
    prompt = {
        ...
        "completion": f"Complete this equation with only the numerical result: {a} {symbol} {b} =",
        "named": f"Left operand: {a}. Right operand: {b}. Operation: {op}. Return only the numerical result.",
        "reversed": f"The right operand is {b}; the left operand is {a}. Compute left {word} right. Return only the numerical result.",
        "distractor": f"Ticket number: 731. Ignore the ticket number. Compute {a} {word} {b}. Return only the numerical result.",
    }[style]
    return (
