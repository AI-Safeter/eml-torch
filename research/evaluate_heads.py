...
def freeze(out, directories):
    ...
    result = {
        ...
        "evaluation": {
            ...
            "stress": "Unseen instruction format, shifted operands, and addition carry-pattern holdout",
...
class Replacements:
    ...
    @contextmanager
    def hook(self, model, kind, mixed=None):
        cache = {}

        def post(module, args, out):
            # Generation invokes this at subsequent single-token decode positions too.
            if args[0].shape[1] <= 1:
                return out
...
def ordinary(model, tok, rep, rows, batch_size=16):
    ...
    for start in range(0, len(rows), batch_size):
        ...
        for kind in rep.kinds:
            if kind == "original":
                ...
                with rep.hook(model, kind) as current:
                ...
                records[kind].append(
                    {
                        **row_metadata(row, start + i),
                        ...
                        "first_digit_correct": bool(scores[i].argmax() == ct[i]),
                        "kl": float(kl[i]),
...
def interventions(model, tok, rep, rows, batch_size=16):
    records = {kind: [] for kind in rep.kinds}
    for start in range(0, len(rows), batch_size):
        batch = rows[start : start + batch_size]
