...
- **Anti-tautology guards (pre-reg locked, run 2026-05-22).** Three controls applied to the headline formula:
  - **Inter-circuit cross-fit**: factual → induction R²≈-10⁷⁰ (explodes); copy_oneshot R²=-29.8; ioi R²=-1.4; **syntactic R²=+0.75 partial transfer** (both circuits share L=0 + overlapping entropy distribution). Formula is not generic-noise-floor and not pure-factual-mechanism — it captures completion-confidence at L=0.
  - **Random-target re-measurement** (re-ran Qwen3.6 on 50 factual prompts with random non-self capitals as targets; true P(random)≈0): formula still predicts 0.44 mean → R²=-1.1×10¹³. The formula is **target-canonical** — predicts answer-confidence assuming canonical target; doesn't use log_tok_id at all. Not a prompt-statistical noise floor.
  - **Vendor cross-fit** (Qwen formula on Gemma-4 factual): R²=-65 → **vendor-specific signature confirmed**.
- **Refined post-guards verdict**: the formula is a closed-form predictor of Qwen3.6-27B's *factual-recall confidence* as a function of (L, entropy_top50) — target-canonical, vendor-specific, partially circuit-specific. Not "which token will the model pick"; "how confident on a canonical-target factual completion." The cert pair stands for what it claims at τ=0.10 (interval-bound-implied per filter #12).
